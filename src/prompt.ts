...
export function renderSkillRules(skillsDir: string): string {
  return [
    '# 技能',
    '',
    `技能是 \`${skillsDir}/<名字>/SKILL.md\` 下的一个文件。用你的文件工具直接写进去，`,
    'dsh 会自己发现它，不需要重启、不需要登记。',
    '',
    '## 文件格式',
    '',
    '```markdown',
    '---',
    'name: 类级别的名字（跟目录名一致）',
    'description: 一句话说清「什么时候该用」——模型只靠这一行判断要不要加载它',
    `metadata:`,
    ...
    '```',
    '',
    `**\`metadata: generated-by: dsh-behuman\` 这两行必须原样带上。**它标明这个技能是自动写的，`,
    '不是人手工写的——这决定以后能不能自动更新它，也决定用户看到时知不知道该不该动。',
    '少了这两行，这个技能就会被当成手写的，**以后再也不会被自动改进**。',
    '',
    '## 正文该写什么',
    '',
    '- **步骤和命令**——具体到能照着敲，别写"配置好环境"这种废话',
    '- **关键参数和取值**——为什么是这个值，换了会怎样',
    '- **踩过的坑和绕法**——写【怎么绕过去】，不是【这个坑多烦】',
    '- **什么时候不适用**——避免下次生搬硬套',
    '',
    '## 什么时候写',
    '',
    ...
    '',
    '**没解决的问题不要写。**没跑通就记下来的"方法"，是披着方法外衣的死胡同。',
    '',
    '## 什么时候改',
    '',
    '- 用技能时发现它过时了、缺步骤、或者根本是错的 → **立刻补它**，别等着被要求',
    '- **改之前先看有没有 `generated-by: dsh-behuman`**：',
    '  - **有** → 是自动生成的，放心改',
    '  - **没有** → **是人手写的，不要动它**。人手写的技能背后有你看不到的理由',
    '- 不被维护的技能会变成负债',
    '',
    '## 技能库的目标形状',
    '',
    '- **类级别的上位技能**（一个类一个），配 `references/` 子目录放会话级细节',
    '- 不要写成一长串「一次任务一个」的窄技能',
    '',
    '## 命名红线',
    '',
...
export function renderReviewPrompt(
  ...
  return [
    ...
    '',
    '**不要改人手写的技能。**改之前先看它有没有 `generated-by: dsh-behuman`：',
    '有就说明是自动生成的，可以改；没有就是人写的，**碰都不要碰**，换个名字新建。',
    '',
    '技能库现有的技能（不要重复造）：',
    skillBlock,
    '',
